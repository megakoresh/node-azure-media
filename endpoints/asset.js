...
var calls = {
	...
	removeAllLocatorsOfType: function(id, type, cb) {
		cb = cb || function () {};
		
		this.rest.asset.listLocators(id, function(err, locators){
			if(!err){
				async.each(locators, function(locator, removedLocator){
					if(locator.Type == type){
						this.rest.locator.delete(locator.toJSON().Id, removedLocator);
					} else {
						removedLocator();
					}
				}.bind(this), function allLocatorsRemoved(err){
					cb(err);
				});
			} else {
				cb(err);
